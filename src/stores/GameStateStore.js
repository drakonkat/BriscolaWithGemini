...
import { getLocalAIMove, getFallbackWaifuMessage } from '../core/localAI.js';
import { getAIWaifuTrickMessage, getAIGenericTeasingMessage, QuotaExceededError } from '../core/gemini.js';
...
export class GameStateStore {
    ...
    startGame(param) { throw new Error('startGame must be implemented by subclass'); }
    resolveTrick() { throw new Error('resolveTrick must be implemented by subclass'); }
    handleEndOfGame() { throw new Error('handleEndOfGame must be implemented by subclass'); }
    getCardPoints(card) { throw new Error('getCardPoints must be implemented by subclass'); }
    ...
    selectCardForPlay(card, activatePower) {
    ...
    drawCards(trickWinner) {
        const drawAndContinue = () => {
            runInAction(() => {
                const drawOneCard = (player) => {
    ...
    getWaifuMessage = async (humanCard, aiCard, points, winner) => {
    ...
    handleDragStart(card, e) { /* Base implementation */ }
    handleDragMove(e, zones) { /* Base implementation */ }
    handleDragEnd() { /* Base implementation */ }


    // FIX: Made save/load methods abstract to enforce implementation in subclasses.
    saveGame() { throw new Error('saveGame must be implemented by subclass'); }
    loadGame() { throw new Error('loadGame must be implemented by subclass'); }
    clearSavedGame() { throw new Error('clearSavedGame must be implemented by subclass'); }
    resumeGame() { throw new Error('resumeGame must be implemented by subclass'); }
    get hasSavedGame() { throw new Error('hasSavedGame must be implemented by subclass'); }
