...
const DAILY_MISSIONS_DEFINITIONS = [
...
const WEEKLY_MISSIONS_DEFINITIONS = [
...
export class MissionStore {
    ...
    missionProgress = loadFromLocalStorage('mission_progress', {});
    achievementProgress = loadFromLocalStorage('achievement_progress', {});

    lastDailyReset = loadFromLocalStorage('last_daily_reset', 0);
    lastWeeklyReset = loadFromLocalStorage('last_weekly_reset', 0);

    uniqueProgress = loadFromLocalStorage('unique_progress', {});
    ...
    get hasUnclaimedRewards() {
        const checkList = (missions, progress) => {
    ...
    claimReward(missionId) {
        ...
        if (state && !state.claimed && state.progress >= mission.target) {
            Object.entries(mission.rewards).forEach(([type, amount]) => {
                ...
                else if (rewardType.endsWith('_shards')) this.rootStore.gachaStore.addShards(rewardType.split('_')[0].toUpperCase(), amount);
                else if (rewardType.endsWith('_essences')) this.rootStore.gachaStore.addEssences(rewardType.split('_')[0], amount);
