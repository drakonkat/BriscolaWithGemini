...
export class UIStateStore {
    ...
    isDifficultyDetailsOpen = loadFromLocalStorage('ui_difficulty_details_open', window.innerWidth > 768);
    isWaifuDetailsOpen = loadFromLocalStorage('ui_waifu_details_open', window.innerWidth > 768);
    ...
    setIsQuotaExceededModalOpen = (isOpen) => {
    ...
    setDrawingCards = (cards) => {
        this.drawingCards = cards;
    }

    handleSubscriptionInterest = (vote) => {
    ...
    setHasVotedForSubscription = (voted) => {
    ...
    setTutorialStep(stepId) {
        ...
        this.tutorialText = T[details.textKey];
    ...
    setHighlightedElementByQuery(query, stepId) {
        setTimeout(() => {
            runInAction(() => {
                if (query) {
