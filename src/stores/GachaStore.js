...
export const DUNGEON_REWARD_BACKGROUNDS = {
...
export class GachaStore {
    rootStore;
    waifuCoins = loadFromLocalStorage('waifu_coins', 100);
    r_shards = loadFromLocalStorage('r_shards', 0);
    sr_shards = loadFromLocalStorage('sr_shards', 0);
    ssr_shards = loadFromLocalStorage('ssr_shards', 0);
    fire_essences = loadFromLocalStorage('fire_essences', 0);
    water_essences = loadFromLocalStorage('water_essences', 0);
    air_essences = loadFromLocalStorage('air_essences', 0);
    earth_essences = loadFromLocalStorage('earth_essences', 0);
    // FIX: Add missing transcendental_essences property.
    transcendental_essences = loadFromLocalStorage('transcendental_essences', 0);
    r_keys = loadFromLocalStorage('r_keys', 0);
    sr_keys = loadFromLocalStorage('sr_keys', 0);
    ssr_keys = loadFromLocalStorage('ssr_keys', 0);
    unlockedBackgrounds = loadFromLocalStorage('unlocked_backgrounds', []);
    unlockedDungeonBackgrounds = loadFromLocalStorage('unlocked_dungeon_backgrounds', []);
    hasRolledGacha = loadFromLocalStorage('has_rolled_gacha', false);
    fullscreenImage = '';
    isRolling = false;
    gachaAnimationState = { active: false, rarity: null };
    isCraftingMinigameOpen = false;
    craftingAttempt = null;

    lastDungeonRunRewards = { coins: 0, shards: { R: 0, SR: 0, SSR: 0 }, unlockedBackground: null };
    lastDungeonMatchRewards = { coins: 0, shards: { R: 0, SR: 0, SSR: 0 } };

    lastGachaResult = null;
    multiGachaResults = [];
    lastMultiGachaShards = { R: 0, SR: 0, SSR: 0 };
    ...
    addTranscendentalEssences = (amount) => {
    ...
    handleGachaRoll = async () => {
        ...
        let rarityToPull = rand < 0.05 ? 'SSR' : rand < 0.20 ? 'SR' : 'R';
        ...
        if(isDuplicate) {
            ...
            playSound(`gacha-unlock-${pulledItem.rarity.toLowerCase()}`);
    ...
    handleMultiGachaRoll = async () => {
        ...
        let newUnlocks = [];
        let totalShards = { R: 0, SR: 0, SSR: 0 };
    ...
    resolveCraftingAttempt = (result) => {
        ...
        if (result === 'failure') {
            ...
            if ('essences' in cost) {
                ...
                const essenceTypes = ['fire_essences', 'water_essences', 'air_essences', 'earth_essences'];
            ...
            if (result === 'critical') {
                ...
                playSound(`gacha-unlock-${rarity.toLowerCase()}`);
