...
import { WAIFUS, BOSS_WAIFU } from '../core/waifus.js';
import { getImageUrl, shuffleDeck, getCardPoints } from '../core/utils.js';
import { createDeck } from '../core/classicGameLogic.js';
import { initializeRoguelikeDeck, getRoguelikeTrickWinner, determineWeaknessWinner, calculateRoguelikeTrickPoints } from '../core/roguelikeGameLogic.js';
import { ALL_POWER_UP_IDS, POWER_UP_DEFINITIONS } from '../core/roguelikePowers.js';
import { ROGUELIKE_REWARDS } from '../core/constants.js';
...
export class RoguelikeModeStore extends GameStateStore {
    roguelikeState = this._loadRoguelikeState();
    powerSelectionOptions = null;
    activeElements = [];
    isElementalChoiceOpen = false;
    cardForElementalChoice = null;
    elementalClash = null;
    powerAnimation = null;
    ...
    trickResolutionCallback = null;

    activatedElementsThisMatch = [];
    ...
    selectPowerUp = (powerId, isUpgrade) => {
        if (isUpgrade) {
        ...
        runInAction(() => {
            ...
            if (acePower && this.briscolaSuit) {
                ...
                if (acePower.level === 1) { // Asso, 3, or Re
                    ...
                    cardsToGive.push(['Asso', '3', 'Re'][Math.floor(Math.random() * 3)]);
                } else if (acePower.level === 2) { // Asso or 3
                    // FIX: Cast the selected string to `Value` to satisfy TypeScript's type checker.
                    cardsToGive.push(['Asso', '3'][Math.floor(Math.random() * 2)]);
    ...
    confirmElementalChoice = (activate, card) => {
        ...
        if (!hadSwap) {
            if (humanCardOnTable?.elementalEffectActivated) {
                ...
                for (const card of this.aiHand) {
                    if (card.element) {
                        const isWinningTrick = !humanCardOnTable || getRoguelikeTrickWinner([humanCardOnTable, card], this.trickStarter, this.briscolaSuit, this.roguelikeState.activePowers) === 'ai';
    ...
    resolveTrick() {
        ...
        const newHistoryEntry = { trickNumber: this.trickCounter + 1, humanCard, aiCard, winner: trickWinner, points: totalPoints, clashResult: this.elementalClash || undefined, basePoints, bonusPoints, bonusPointsReason: bonusReasons.join(', ') };
        ...
        if (this.humanHand.length === 0 && this.aiHand.length === 0 && !this.isResolvingTrick) {
            ...
            if (winner === 'human') {
                ...
                Object.entries(essenceCounts).forEach(([element, count]) => {
                    if (count > 0) {
                        this.rootStore.gachaStore.addEssences(element, count);
                        const rewardType = `${element}_essences`;
    ...
    activateFollowerAbility(waifuName) {
        ...
        const abilityName = (this.T)[abilityId];
        ...
        if (abilityUsed) {
            const abilityEntry = { isAbilityUse: true, trickNumber: this.trickCounter + 1, waifuName, abilityName };
