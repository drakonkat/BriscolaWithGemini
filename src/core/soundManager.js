...
const playNote = (context, frequency, startTime, duration, volume = 0.5) => {
...
const createSnare = (context, time) => {
    if (!masterGainNode) return;
...
const createHihat = (context, time, isOpen) => {
    if (!masterGainNode) return;
...
const createGuitarChord = (context, time, frequencies) => {
    if (!masterGainNode) return;
    const duration = 0.8;
...
const createReverbImpulseResponse = (context) => {
...
export const updateSoundSettings = (newSettings) => {
...
export const startMusic = (settings) => {
    const context = initAudioContext();
