...
import { getCardId, getImageUrl } from '../../core/utils.js';
...
export const GameBoard = observer(() => {
    ...
    const normalZoneRef = useRef(null);
    const powerZoneRef = useRef(null);
    const cancelZoneRef = useRef(null);

    // Mode-specific access
    const isRoguelike = gameplayMode === 'roguelike';
    const isDungeon = gameplayMode === 'dungeon';
    const roguelikeStore = isRoguelike ? gameStateStore : null;
    const dungeonStore = isDungeon ? gameStateStore : null;
        ...
        <main className="game-board" data-tutorial-id="end-tutorial">
            ...
            {isRoguelike && roguelikeStore?.isElementalChoiceOpen && roguelikeStore.cardForElementalChoice && (
                <div className="elemental-choice-modal">
                    ...
                    <div className="elemental-choice-options">
                        ...
                        <button onClick={() => roguelikeStore.confirmElementalChoice('water')}>Water</button>
                        <button onClick={() => roguelikeStore.confirmElementalChoice('air')}>Air</button>
                        <button onClick={() => roguelikeStore.confirmElementalChoice('earth')}>Earth</button>
                    </div>
                    <button onClick={roguelikeStore.cancelElementalChoice}>{T.cancel || 'Cancel'}</button>
                </div>
            )}
                        ...
                        {isDiceRolling ? (
                            <div className="clash-rolling-container">
                                <div className="clash-cards-preview">
                                    {humanCardForClash && <CardView card={humanCardForClash} lang={language} cardDeckStyle={cardDeckStyle} />}
                                    {aiCardForClash && <CardView card={aiCardForClash} lang={language} cardDeckStyle={cardDeckStyle} />}
                                </div>
                            ...
                            <>
                                ...
                                <div className="clash-results">
                                    <div className={`clash-player-result ${elementalClash.winner === 'human' ? 'winner' : elementalClash.winner === 'tie' ? 'tie' : ''}`}>
                                        <h3>{T.scoreYou}</h3>
                                        {humanCardForClash && <CardView card={humanCardForClash} lang={language} cardDeckStyle={cardDeckStyle} />}
                                        {elementalClash.type === 'dice' && <div className="clash-roll">{elementalClash.humanRoll}</div>}
                                    </div>

                                    <div className={`clash-player-result ${elementalClash.winner === 'ai' ? 'winner' : elementalClash.winner === 'tie' ? 'tie' : ''}`}>
                                        <h3>{currentWaifu?.name ?? 'AI'}</h3>
                                        {aiCardForClash && <CardView card={aiCardForClash} lang={language} cardDeckStyle={cardDeckStyle} />}
                                        {elementalClash.type === 'dice' && <div className="clash-roll">{elementalClash.aiRoll}</div>}
                                    </div>
                                </div>
                                <div className="clash-outcome">
                                    {elementalClash.winner === 'tie' ? T.elementalClash.tie : `${T.elementalClash.winner}! ${elementalClash.winner === 'human' ? T.scoreYou : currentWaifu?.name}`}
                                </div>
                            </>
                        )}
                    </div>
            ...
            <div className="table-area">
                <div className="deck-area-wrapper" data-tutorial-id="briscola-deck">
                    {currentModifier && currentModifier.id !== 'NONE' && (
                        <button className="dungeon-modifier-display" title={currentModifier.description} onClick={() => uiStore.openModal('dungeonModifierInfo')}>
                            <span>{currentModifier.name}</span>
                        </button>
                    )}
            ...
            <div className="bottom-bar">
                ...
                {lastTrick && !isPreviewHiddenInClassic && (
                    <button className="last-trick-recap" onClick={() => uiStore.openModal('history')} title={historyButtonTitle}>
