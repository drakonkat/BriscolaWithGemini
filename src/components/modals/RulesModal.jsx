...
export const RulesModal = ({ isOpen, onClose, language, difficulty }) => {
    if (!isOpen) {
        return null;
    }
        ...
        <div className="game-over-overlay" onClick={onClose}>
            <div className="rules-modal" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="roguelike-rewards-rules-grid">
                    {(['easy', 'medium', 'hard', 'nightmare', 'apocalypse']).map(diff => {
                        const rewards = ROGUELIKE_REWARDS[diff];
                        const difficultyKey = `difficulty${diff.charAt(0).toUpperCase() + diff.slice(1)}`;
                        return (
                            <div key={diff} className={`reward-difficulty-column difficulty-${diff}`}>
                                <strong>{T[difficultyKey]}</strong>
                                <ul>
                ...
                <div className="roguelike-powers-rules-list">
                    {ALL_POWER_UP_IDS.map(powerId => {
                        const powerDef = POWER_UP_DEFINITIONS[powerId];
                        return (
