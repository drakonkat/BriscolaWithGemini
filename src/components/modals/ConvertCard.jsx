...
export const ConvertCard = ({
    ...
    if (rarity === 'from-r' || rarity === 'from-sr') {
        return (
            <div className={`crafting-card ${rarity ? `rarity-${rarity}` : ''}`}>
                <div className="crafting-card-header">
                    <h3>{title}</h3>
                </div>
                <div className="crafting-shard-info">
                    <span>{shardType === 'r' ? T_gallery.shardLabelR(gachaStore.r_shards) : T_gallery.shardLabelSR(gachaStore.sr_shards)}</span>
                </div>
        ...
        return (
            <div className={`crafting-card ${rarity ? `rarity-${rarity}` : ''}`}>
                <div className="crafting-card-header">
                    <h3 className={rarity === 'from-elemental' ? 'elemental-conversion-rule' : ''}>{title}</h3>
                    {rarity === 'from-elemental' && <span>{T_gallery.convertEssencesTitle}</span>}
                </div>
